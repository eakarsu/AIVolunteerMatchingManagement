...
export default function CfMobileCompanionAppForShiftCheckIn() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-mobile-companion-app-for-shift-check-in/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-mobile-companion-app-for-shift-check-in', project: 'AIVolunteerMatchingManagement' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-mobile-companion-app-for-shift-check-in/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Mobile companion app for shift check-in</h1>
