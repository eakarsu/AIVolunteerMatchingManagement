...
export default function CfSkillsToOpportunityMatchingEngineWithExplainability() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-skills-to-opportunity-matching-engine-with-explainability/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-skills-to-opportunity-matching-engine-with-explainability', project: 'AIVolunteerMatchingManagement' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-skills-to-opportunity-matching-engine-with-explainability/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Skills-to-opportunity matching engine with explainability</h1>
