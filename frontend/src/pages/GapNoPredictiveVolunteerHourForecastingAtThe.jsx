...
export default function GapNoPredictiveVolunteerHourForecastingAtThe() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-predictive-volunteer-hour-forecasting-at-the-organization-level/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-predictive-volunteer-hour-forecasting-at-the-organization-level', project: 'AIVolunteerMatchingManagement' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-predictive-volunteer-hour-forecasting-at-the-organization-level/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No predictive volunteer-hour forecasting at the organization level</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVolunteerMatchingManagement</p>
      </div>
