...
// Feature: Background check integrations (Sterling, Checkr)
...
export default function CfBackgroundCheckIntegrationsSterlingCheckr() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-background-check-integrations-sterling-checkr/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-background-check-integrations-sterling-checkr', project: 'AIVolunteerMatchingManagement' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-background-check-integrations-sterling-checkr/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Background check integrations (Sterling, Checkr)</h1>
