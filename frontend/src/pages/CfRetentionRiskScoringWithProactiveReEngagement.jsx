...
// Feature: Retention risk scoring with proactive re-engagement campaigns
...
export default function CfRetentionRiskScoringWithProactiveReEngagement() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-retention-risk-scoring-with-proactive-re-engagement-campaigns/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-retention-risk-scoring-with-proactive-re-engagement-campaigns', project: 'AIVolunteerMatchingManagement' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-retention-risk-scoring-with-proactive-re-engagement-campaigns/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Retention risk scoring with proactive re-engagement campaigns</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVolunteerMatchingManagement</p>
      </div>
