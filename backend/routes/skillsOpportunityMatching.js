...
// Skills-to-opportunity matching engine with explainability
// Feature: skills-opportunity-matching
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Skills-to-opportunity matching engine with explainability.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'skills-opportunity-matching', payload, result);
    res.json({ success: true, feature: 'skills-opportunity-matching', ...result });
  } catch (err) {
    console.error('skills-opportunity-matching analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Skills-to-opportunity matching engine with explainability. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'skills-opportunity-matching', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'skills-opportunity-matching']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'skills-opportunity-matching', title: 'Skills-to-opportunity matching engine with explainability', project: 'AIVolunteerMatchingManagement' });
});
