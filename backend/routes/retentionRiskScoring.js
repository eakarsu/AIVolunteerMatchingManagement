...
// Retention risk scoring with proactive re-engagement campaigns
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'retention-risk-scoring', payload, result);
    res.json({ success: true, feature: 'retention-risk-scoring', ...result });
  } catch (err) {
    console.error('retention-risk-scoring analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Retention risk scoring with proactive re-engagement campaigns. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'retention-risk-scoring', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'retention-risk-scoring', title: 'Retention risk scoring with proactive re-engagement campaigns', project: 'AIVolunteerMatchingManagement' });
});
