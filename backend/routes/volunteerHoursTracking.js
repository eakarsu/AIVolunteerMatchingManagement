...
// Volunteer hour tracking with verification workflow
// Feature: volunteer-hours-tracking
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'volunteer-hours-tracking', payload, result);
    res.json({ success: true, feature: 'volunteer-hours-tracking', ...result });
  } catch (err) {
    console.error('volunteer-hours-tracking analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Volunteer hour tracking with verification workflow. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'volunteer-hours-tracking', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'volunteer-hours-tracking', title: 'Volunteer hour tracking with verification workflow', project: 'AIVolunteerMatchingManagement' });
});
