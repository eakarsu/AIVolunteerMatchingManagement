...
// Impact report auto-generation for grant submissions
// Feature: impact-report-generation
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'impact-report-generation', payload, result);
    res.json({ success: true, feature: 'impact-report-generation', ...result });
  } catch (err) {
    console.error('impact-report-generation analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Impact report auto-generation for grant submissions. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'impact-report-generation', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'impact-report-generation', title: 'Impact report auto-generation for grant submissions', project: 'AIVolunteerMatchingManagement' });
});
